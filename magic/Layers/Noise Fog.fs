/*{
    ...
    "DESCRIPTION": "Mod based on Jazz Universe by olav",
    "IMPORTED": {
    },
    "INPUTS": [
        {
...
#define RAYS_COUNT 8
#define STEP_MODIFIER 1.1
#define SHARPNESS 0.01
#define NOISE_LAYERS_COUNT 2.0
#define JITTERING 0.2
...
vec3 fogMarch(vec3 rayStart, vec3 rayDirection, float time, float disMod)
{
    ...
    for(int i = 0; i < RAYS_COUNT; i++)
    {
        ...
        fog += volumetricFog(point, disMod) //intensity
            * mix(c1.rgb, c2.rgb * (1.0 + disMod * 0.5), getNoiseFromVec3((point + vec3(12.51, 52.167, 1.146)) * 0.5)) //coloring
...
void main() {
   ...
   // float musicVolume = getBeat();
    ...
    //postprocess
    fog *= 2.5 * bright;
    fog += 0.07 * mix(c1.rgb, c2.rgb, 0.5);	//Colouring the darkness
