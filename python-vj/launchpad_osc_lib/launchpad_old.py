...
This module now uses the lpminimk3 library for core device operations,
focusing on OSC-specific abstractions and async API.
"""
...
try:
    ...
    lpminimk3 = None  # type: Any
    find_launchpads = None  # type: Any
    LaunchpadMiniMk3 = None  # type: Any
    Mode = None  # type: Any
    ButtonEvent = None  # type: Any

# Keep mido available for backward compatibility with other code
try:
    import mido
...
def find_launchpad_ports() -> Tuple[Optional[str], Optional[str]]:
    ...
    lpminimk3.find_launchpads() internally. For new code, consider using
    find_launchpads() directly from lpminimk3.
    ...
    try:
        devices = find_launchpads()
        if devices:
            device = devices[0]
            # lpminimk3 handles ports internally, return device ID as pseudo-ports
            return device.id, device.id
        else:
            logger.warning("Launchpad Mini Mk3 not found")
            return None, None
    except Exception as e:
        logger.warning(f"Could not query Launchpad devices: {e}")
        return None, None
