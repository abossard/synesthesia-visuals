...
void main() 
{
	...
	vec2 position = ( gl_FragCoord.xy / RENDERSIZE.xy )-vec2(posX, posY);
	position.y *= ratio;
	vec3 p = position.xyx*zoom;
    
	//p.z = 2.0*zoom*0.5;
	vec3 color = vec3(0.0);//computeColor(wolfFaceEQ(p+vec3(7.0, -1.0, 0.2),TIME));
	p.z = 0.0;
	int selection = int(equation);
	
	if (selection == 0) {
    color = computeColor(dogFaceEQ(p, TIME));
    } else if (selection == 1) {
        color = computeColor(mandelbrotEQ(p, TIME));
    } else if (selection == 2) {
        color = computeColor(triangleEQ(p, TIME));
    } else if (selection == 3) {
        color = computeColor(crossEQ(p, TIME));
    } else if (selection == 4) {
        color = computeColor(quadTorusEQ(p, TIME));
    } else if (selection == 5) {
        color = computeColor(bretzTrifolEQ(p, TIME));
    } else if (selection == 6) {
        color = computeColor(bretzel6EQ(p, TIME));
    } else if (selection == 7) {
        color = computeColor(bretzBernEQ(p, TIME));
    } else if (selection == 8) {
        color = computeColor(pieuvreEQ(p, TIME));
    }
