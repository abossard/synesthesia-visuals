/*{
  ...
  "DESCRIPTION" : "Automatically converted from https:\/\/www.shadertoy.com\/view\/llcXWM by tdhooper.",
  "INPUTS" : [
    {
      "NAME" : "rotateX",
      "TYPE" : "float",
...
vec3 spectrum(float n) {
    return pal( n, vec3(0.5,0.5,0.5),vec3(0.5,0.5,0.5),vec3(1.0,1.0,1.0),vec3(colR,colG,colB) );
}
...
mat3 combinedRotation(float rx, float ry, float rz) {
    mat3 rotX = rotationX(rx);
    mat3 rotY = rotationY(ry);
    mat3 rotZ = rotationZ(rz);

    // Apply rotations in ZYX order
    return rotZ * rotY * rotX;
}

mat3 sphericalMatrix(float theta, float phi, float gamma) {
    float cx = cos(theta);
    float cy = cos(phi);
    float sx = sin(theta);
    float sy = sin(phi);
    ...
    mat3 rotZ = rotationZ(gamma);

    // Original spherical matrix
    mat3 sphereMat = mat3(
        cy, -sy * -sx, -sy * cx,
        0, cx, sx,
        sy, cy * -sx, cy * cx
    ...
    // Combine spherical rotation with Z-axis rotation
    return rotZ * sphereMat;
}
mat3 modelRotation() {
    // Define rotation angles
    float rx = 0.001 + rotateX * PI * 2.;  // Rotation angle around the X-axis
    float ry = 0.001 + rotateY * PI * 2.;  // Rotation angle around the Y-axis
    float rz = 0.001 + rotateZ  * PI * 2.;  // Rotation angle around the Z-axis

    // Create the combined rotation matrix
    return combinedRotation(rx, ry, rz);
}
...
void main() {
    ...
    vec2 m = 1.0 / RENDERSIZE.xy;
    vec3 camPos = vec3( 0., 0., 2.);
    vec3 camTar = vec3( 0. , 0. , 0. );
    float camRoll = 0.;
    
    // camera movement
    doCamera(camPos, camTar, camRoll, TIME * rate1 * 0.3, m);
