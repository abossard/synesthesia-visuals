// SaturdayShader Week 25 : HoopLoop
// by Joseph Fiola (http://www.joefiola.com)
// 2016-02-06


/*{
  "CREDIT": "Joseph Fiola",
  "DESCRIPTION": "",
  "CATEGORIES": [
    "Generator"
  ],
  "INPUTS": [
    {
      "NAME": "invert",
      "TYPE": "bool",
      "DEFAULT": 0
    },
    {
      "NAME": "zoom",
      "TYPE": "float",
      "DEFAULT": 2,
      "MIN": 1,
      "MAX": 8
    },
    {
      "NAME": "size",
      "TYPE": "float",
      "DEFAULT": 0.5,
      "MIN": 0,
      "MAX": 0.5
    },
    {
      "NAME": "lineEffect",
      "TYPE": "float",
      "DEFAULT": 0.0137,
      "MIN": 0,
      "MAX": 0.037
    },
    {
      "NAME": "patternOffset",
      "TYPE": "float",
      "DEFAULT": 0,
      "MIN": 0,
      "MAX": 1
    },
    {
      "NAME": "rSin",
      "TYPE": "float",
      "DEFAULT": 0.5,
      "MIN": 0,
      "MAX":0.5
    },
    {
      "NAME": "xCos",
      "TYPE": "float",
      "DEFAULT": 1,
      "MIN": 0,
      "MAX": 1
    },
    {
      "NAME": "modAll",
      "TYPE": "float",
      "DEFAULT": 1,
      "MIN": 0,
      "MAX": 1
    },
    {
      "NAME": "ySin",
      "TYPE": "float",
      "DEFAULT": 0,
      "MIN": 0,
      "MAX": 1
    },
    {
      "NAME": "blur",
      "TYPE": "float",
      "DEFAULT": 0.005,
      "MIN": 0.001,
      "MAX": 0.005
    },
    {
      "NAME": "function",
      "TYPE": "long",
      "VALUES": [
        0,
        1
      ],
      "LABELS": [
        "abs",
        "fract"
      ],
      "DEFAULT": 1
    },
    {
      "NAME": "rotate",
      "TYPE": "float",
      "DEFAULT": 0,
      "MIN": -1,
      "MAX": 1
    },
    {
      "NAME": "pos",
      "TYPE": "point2D",
      "DEFAULT": [
        0.5,
        0.5
      ],
      "MIN": [
        0,
        0
      ],
      "MAX": [
        1,
        1
      ]
    }
  ]
}*/


const int   NUM_CIRCLES 	= 33;
const float thickness = 0.002;

#define PI 3.14159265358979323846
#define TWO_PI 6.28318530718

vec3 drawCircle(vec2 p, vec2 center, float radius, float edgeWidth, vec3 color)
{
    float dist = length(p - center);
    vec3 ret;
    
    float look;
    if (function == 0) look = abs(dist -size);
    else if (function == 1) look = fract(dist -size);

	ret = color * (1.0 - lineEffect - smoothstep(radius, (radius+edgeWidth),  look  ));
   
    return ret;
} 

vec3 invertColor(vec3 color) {
    return vec3(color *-1.0 + 1.0);
}

//rotation function
vec2 rot(vec2 uv,float a){
	return vec2(uv.x*cos(a)-uv.y*sin(a),uv.y*cos(a)+uv.x*sin(a));
}


void main()
{

	vec2 uv = gl_FragCoord.xy / RENDERSIZE.xy;
	uv -= vec2(pos);
	uv.x*=RENDERSIZE.x/RENDERSIZE.y;
	uv *= zoom;
	uv += vec2(0.0, ySin * modAll* 0.5);
	
	uv=rot(uv,rotate * PI);
	
    
	vec3 color = vec3(0.0);
	float angleIncrement = TWO_PI / float(NUM_CIRCLES);


	for (int i = 0; i < NUM_CIRCLES; ++i) {
		float t = angleIncrement*(float(i));
		float r = sin(rSin * modAll * t * 2.); 	// In VDMX I use the following line and control the "animate" slider
		//float r = sin(rSin * t + animate);
		vec2 p = vec2(r*cos(t*xCos * modAll), r*sin(t*ySin * modAll));
		
		uv=rot(uv,patternOffset * PI * 0.5);
		
        if (lineEffect >= 0.2) color = invertColor(color);
       	
       	color += drawCircle(uv, p, thickness, blur, vec3(1.0));
	}
	
	if (invert) color = invertColor(color);

	gl_FragColor = vec4(color,1.0);
}