def render_ai(fb, audio, frame, lut, state):
    ...
    # Use pre-smoothed bands from SmoothedAudio (no raw mel_bands slicing)
    bass = audio.band_kick
    mid = audio.band_mid
    high = audio.band_high

    # Kick pulse drives jumps (shaped envelope, not binary beat)
    if audio.kick_pulse > 0.7:
        pulse_strength = audio.kick_pulse
        for worm in state['worms']:
            worm['jump'] = max(worm['jump'], 1.5 + pulse_strength * 1.5)
            worm['vx'] += (np.random.random() - 0.5) * 0.2 * pulse_strength
            worm['vy'] += (np.random.random() - 0.5) * 0.2 * pulse_strength
    ...
    for i, worm in enumerate(state['worms']):
        ...
        # Drift with audio flux (already smoothed)
        ...
        for seg in range(num_segments):
            ...
            # Undulating motion — mid drives speed (already smooth)
        ...
        # Color mapping with audio reactivity (all pre-smoothed)
