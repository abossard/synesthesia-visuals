...
def build_vc(client, ids):
    ...
    for (_, _, _, _, _, cat_key, lp_row) in frames_def:
        ...
        if cat_key == "movement":
            ...
            # First 6 are regular function buttons, last 2 are special (blackout + stopall)
            func_ids = ids["utils"][:6]  # Haze Low/Full/Off, UV Full/Strobe/Off
        elif cat_key in ids:
        ...
        btn_items = []
        if cat_key == "utils":
            # 6 function buttons + blackout + stopall
            for col_idx, fid in enumerate(func_ids):
            ...
            # Blackout button (col 7)
            btn_items.append({
                "parentID": parent_id,
                "x": PAD + 6 * (BTN_W + PAD),
                "y": BTN_Y_OFFSET,
                "width": BTN_W, "height": BTN_H,
                "caption": labels[6] if len(labels) > 6 else "BLACK\nOUT",
                "action": "blackout",
                "bgColor": "#000000", "fgColor": "#ff0000",
            })
            # Stop All button (col 8)
            btn_items.append({
                "parentID": parent_id,
                "x": PAD + 7 * (BTN_W + PAD),
                "y": BTN_Y_OFFSET,
                "width": BTN_W, "height": BTN_H,
                "caption": labels[7] if len(labels) > 7 else "STOP\nALL",
                "action": "stopall",
                "stopAllFadeTime": 1000,
                "bgColor": "#330000", "fgColor": "#ff4444",
            })
        else:
            for col_idx, fid in enumerate(func_ids[:8]):
                label = labels[col_idx] if col_idx < len(labels) else f"Fn {fid}"
                btn_items.append({
                    "parentID": parent_id,
                    "x": PAD + col_idx * (BTN_W + PAD),
